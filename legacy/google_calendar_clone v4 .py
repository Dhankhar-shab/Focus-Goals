            ...
            try:
                ...
                if event_date >= today:
                    for idx, event in enumerate(events):
                        all_events.append((date_str, event_date, idx, event))
            except:
                continue
        
        # Sort by actual date object, then by time if available
        all_events.sort(key=lambda x: (x[1], x[3].get('time', '')))
        
        # Add event cards
        for date_str, event_date, event_idx, event in all_events:
